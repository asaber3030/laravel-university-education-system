      ...
      <PageHeader
        ...
        pageRightComponent={
          <span className="text-dark fw-bold">
            {user.year.title} / {user.year.term_name}
          </span>
        }
      />
          ...
          <Link href={route("students.quizzes.view.subject", subject.id)}>
            ...
            <span>
              ...
              <span className="gray-span">
                {subject.quizzes.length} quiz
                {subject.quizzes.length > 1 ? "zes" : ""}
              </span>
            </span>
          </Link>
        ))}
      ...
      <div className="list-quizzes-tbl">
        <div className="filter-quizzes">
          ...
          <div className="search-handler">
            ...
            />
            <Btn disabled={sub ? false : true} handleClick={applySearch}>
              ...
              {data.map((quiz) => {
                ...
                return (
                  <tr key={quiz.id}>
                    <td>
                      <Link
                        href={route("students.subjects.view", quiz.subject.id)}
                      >
                        {quiz.subject.title}
                      </Link>
                    </td>
                    {quiz.status == 0 && (
                      <td className="text-danger">
                        {QUIZ_STATUS_OBJECT[quiz.status]}
                      </td>
                    )}
                    ...
                    <td>
                      <div className="table-actions">
                        {new Date() <= new Date(quiz.end) && (
                          <Tooltip title="Join">
                            <Link
                              href={route("students.quizzes.join", quiz.id)}
                        ...
                        <Tooltip title="View My Answers">
                          <Link
                            href={route("students.quizzes.view.answers", [
                              quiz.id,
                            ])}
                            className="btn btn-sm btn-dark"
                          >
